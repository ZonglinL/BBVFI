...
class VQFlowNet(pl.LightningModule):
    ...
    def decode(self, quant, x_prev, x_next, phi_list=None):
        ...
        if self.training:
            dec,flow_list = self.decoder(quant, cond_dict)
    ...
    def forward(self, x, x_prev,x_next, return_pred_indices=False):
        ...
        if self.training:
            dec,flow_list = self.decode(quant, x_prev, x_next,phi_list)
        else:
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        flow_gt = self.get_input(batch, 'flow')


        xrec, qloss,flow_list = self(x,x_prev,x_next)

        if optimizer_idx == 0:
            # autoencoder
            aeloss, log_dict_ae = self.loss(qloss, x, xrec, optimizer_idx, self.global_step,
                                            last_layer=self.get_last_layer(), split="train",
                                            flow_list = flow_list,flow_gt=flow_gt,x_prev = x_prev,x_next = x_next)
...
class VQFlowNetInterface(VQFlowNet):
    ...
    def decode(self, h, x_prev, x_next, phi_list, force_not_quantize=False):
